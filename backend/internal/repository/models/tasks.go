...
var (
	taskAllColumns            = []string{"id", "description", "completed"}
	taskColumnsWithoutDefault = []string{"id", "description"}
	taskColumnsWithDefault    = []string{"completed"}
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(taskPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert tasks, could not build conflict column list")
			}
